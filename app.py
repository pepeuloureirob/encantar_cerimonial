...
#
# Localmente, caso DATABASE_URL não exista:
# SQLite -> instance/encantar.db
#
# Também corrige URLs antigas:
# postgres:// -> postgresql://
# ---------------------------------------------------------
...
# Verifica se a conexão ainda está disponível antes de utilizá-la.
...
def cfg(key, default=""):
    ...
    item = Configuracao.query.filter_by(
        chave=key
    ).first()
...
def check_csrf():
    ...
    enviado = request.form.get(
        "_csrf",
        ""
    )

    armazenado = session.get(
        "csrf_token"
    )
...
def depoimentos():
    ...
    if request.method == "POST":
        ...
        if not nome or not mensagem:
            ...
            flash(
                "Preencha seu nome e depoimento.",
                "error"
            )
...
def admin_eventos():
    ...
    if request.method == "POST":
        ...
        # -----------------------------
            ...
            item.titulo = request.form.get(
                "titulo",
                ""
            ).strip()

            item.tipo = request.form.get(
                "tipo",
                ""
            ).strip()

            item.data_evento = request.form.get(
                "data_evento",
                ""
            ).strip()

            item.local = request.form.get(
                "local",
                ""
            ).strip()

            item.descricao = request.form.get(
                "descricao",
                ""
            ).strip()

            item.album_url = request.form.get(
                "album_url",
                ""
            ).strip()
...
def admin_servicos():
    ...
    if request.method == "POST":
        ...
        if action == "delete":
            ...
            item.icone = request.form.get(
                "icone",
                "sparkles"
            ).strip()

            item.ordem = request.form.get(
                "ordem",
                0,
                type=int
            )
...
def admin_equipe():
    ...
    if request.method == "POST":
        ...
        if action == "delete":
            ...
            item.cargo = request.form.get(
                "cargo",
                ""
            ).strip()

            item.bio = request.form.get(
                "bio",
                ""
            ).strip()

            item.instagram = request.form.get(
                "instagram",
                ""
            ).strip()
...
def admin_feedbacks_page():
    ...
    if request.method == "POST":
        ...
        item = Feedback.query.get_or_404(
            item_id
        )
...
def admin_backup():
    ...
    if database_uri.startswith(
        ...
        flash(
            "O banco do site está usando PostgreSQL. O backup do banco deve ser feito pelo próprio provedor do PostgreSQL/Render.",
            "error"
        )
...
# O Render/Gunicorn importa:
#
#     app:app
#
# Portanto precisamos inicializar o banco também
# quando o arquivo é importado pelo Gunicorn.
...
try:
    ...
    # Mostra o erro no log do Render.
    # Isso facilita descobrir problemas de conexão
    # com o PostgreSQL.
